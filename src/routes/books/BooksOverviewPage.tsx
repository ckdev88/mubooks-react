...
const BooksOverviewPage = ({ books, page }: { books: Books; page: string }) => {
	...
	if (
		...
		savedArr = books // TODO: actually might be better to turn
	...
	return books.map((book) => {
		if (page !== 'savedbookspage') {
			...
			const wishlistCheck = savedArr.filter(
				(stateBook) => stateBook.id === book.id && stateBook.wishlist === true
			)
			if (wishlistCheck.length > 0) book.wishlist = true
			else book.wishlist = false
			if (page === 'wishlistpage' && book.wishlist === false) return

			const readingCheck = savedArr.filter(
				(stateBook) => stateBook.id === book.id && stateBook.reading === true
			)
			if (readingCheck.length > 0) book.reading = true
			else book.reading = false
			if (page === 'readingpage' && book.reading === false) return

			const favoriteCheck = savedArr.filter(
				(stateBook) => stateBook.id === book.id && stateBook.favorite === true
			)
			if (favoriteCheck.length > 0) book.favorite = true
			else book.favorite = false
			if (page === 'favoritespage' && book.favorite === false) return

			const finishedCheck = savedArr.filter(
				(stateBook) => stateBook.id === book.id && stateBook.finished === true
			)
			if (finishedCheck.length > 0) book.finished = true
			else book.finished = false
			if (page === 'favoritespage' && book.finished === false) return
		}
