...
const RemoveBookFromXButton = ({
    ...
    // TODO animation check if all dependenies are necessary (book)
    // biome-ignore lint/correctness/useExhaustiveDependencies: <TODO: medium prio>
    const buttonTitle = useMemo(() => {
        if (button_title) return button_title
        return removeType === "move"
            ? `Move back to ${previousListName}`
            : `Remove from ${targetListName}`
    }, [button_title, removeType, previousListName, targetListName, book])
    ...
    // TODO make static pathnames dynamic or via settings.json
    // TODO just improve the whole thing, it's meh
