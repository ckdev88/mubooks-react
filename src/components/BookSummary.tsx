...
const BookSummary = ({ book, currentPage, refer }: { book: Book; currentPage: Page; refer?: Page }) => {
	...
	return (
		<article
			className={book.list > 0 && currentPage === 'search' ? 'book-summary saved' : `book-summary ${currentPage}`}
		>
			<div style={{ marginTop: '-4rem', position: 'absolute' }} id={bookAnchor}></div>
			<div id={`bookSummaryTransitioner${book.id}`} className="transition-wrapper">
